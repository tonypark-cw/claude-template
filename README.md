...
| **스킬** | `.claude/skills/code-review/` | Git diff 기반 코드 리뷰 (APPROVE/WARNING/BLOCK) |
| **스킬** | `.claude/skills/qa-test/` | 프로젝트별 테스트/검증 (API, 회귀) |
| **스킬** | `.claude/skills/review-team/` | 리뷰어+QA 병렬 서브에이전트 오케스트레이터 |
| **스킬** | `.claude/skills/handoff/` | 세션/에이전트 간 컨텍스트 전달 (20줄 핸드오프) |
...
| **문서** | `docs/guides/agent-team.md` | 병렬 멀티 에이전트 개발 가이드 |
| **템플릿** | `templates/agent-team/` | tmux 기반 에이전트 팀 스크립트 + 3개 시나리오 예시 |
...
4. `templates/agent-team/examples/` 를 참고하여 프로젝트에 맞는 에이전트 팀 프롬프트 작성 (선택)
...
| 12 | Auto Agent Team | 비단순 작업 시 자동 팀 구성 |
| 13 | Distributed Context | 포인터 + handoff로 컨텍스트 최소화 |
